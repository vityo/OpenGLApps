...
int main() {
	...
	Shader shaderObjects("../Shaders/objects.vsh", "", "../Shaders/objects.fsh");
	Shader shaderLight("../Shaders/light.vsh", "", "../Shaders/light.fsh");
	Shader shaderBlur("../Shaders/blur.vsh", "", "../Shaders/blur.fsh");
	Shader shaderScreen("../Shaders/screen.vsh", "", "../Shaders/screen.fsh");

	GLuint floorTexture = loadTexture("../Resources/laminat.jpg", true);
	GLuint containerDiffuse = loadTexture("../Resources/box.png", true);
	GLuint containerSpecular = loadTexture("../Resources/box_specular.png", true);
	GLuint containerEmission = loadTexture("../Resources/box_emission.jpg", true);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUniformMatrix4fv(glGetUniformLocation(shaderObjects.program, "view"), 1, GL_FALSE, glm::value_ptr(view));
		glUniformMatrix4fv(glGetUniformLocation(shaderObjects.program, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
		for (GLuint i = 0; i < lightPositions.size(); ++i) {
			glUniform3fv(glGetUniformLocation(shaderObjects.program, ("lights[" + to_string(i) + "].Position").c_str()), 1, &lightPositions[i][0]);
			glUniform3fv(glGetUniformLocation(shaderObjects.program, ("lights[" + to_string(i) + "].Color").c_str()), 1, &lightColors[i][0]);
		}
		glUniform3fv(glGetUniformLocation(shaderObjects.program, "viewPos"), 1, &camera.Position[0]);
		glm::mat4 model = glm::mat4();
		model = glm::translate(model, glm::vec3(0.0f, -1.0f, 0.0));
		model = glm::scale(model, glm::vec3(25.0f, 1.0f, 25.0f));
		glUniformMatrix4fv(glGetUniformLocation(shaderObjects.program, "model"), 1, GL_FALSE, glm::value_ptr(model));
		glUniform1i(glGetUniformLocation(shaderObjects.program, "diffuse"), 0);
		glUniform1i(glGetUniformLocation(shaderObjects.program, "specular"), 1);
		glUniform1i(glGetUniformLocation(shaderObjects.program, "emission"), 2);
		...
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		shaderScreen.use();
		glUniform1i(glGetUniformLocation(shaderScreen.program, "scene"), 0);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, colorBuffers[0]);
		glUniform1i(glGetUniformLocation(shaderScreen.program, "bloomBlur"), 1);
		glActiveTexture(GL_TEXTURE1);
