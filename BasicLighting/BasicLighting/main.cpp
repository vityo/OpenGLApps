...
int main() {
	...
	GLfloat lightRotateDegreeCurrent = 0.0f;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		lightRotateDegreeCurrent += deltaTime * 60.0f;
		glm::vec3 lightPosRotates;
		lightPosRotates.x = cos(glm::radians(lightRotateDegreeCurrent)) * lightPos.x;
		lightPosRotates.y = lightPos.y;
		lightPosRotates.z = sin(glm::radians(lightRotateDegreeCurrent)) * lightPos.z;

		shader.use(); 

		glUniform3f(glGetUniformLocation(shader.program, "lightPos"), lightPosRotates.x, lightPosRotates.y, lightPosRotates.z);
		...
		model = glm::mat4();
		model = glm::translate(model, lightPosRotates);
