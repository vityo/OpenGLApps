...
int main() {
	...
	// текстуры
	GLuint diffuseMap = loadTexture("../Resources/bricks2.jpg");
	GLuint normalMap = loadTexture("../Resources/bricks2_normal.jpg");
	GLuint depthMap = loadTexture("../Resources/bricks2_disp.jpg");
	GLuint diffuseMapToy = loadTexture("../Resources/wood.png");
	GLuint normalMapToy = loadTexture("../Resources/toy_box_normal.png");
	GLuint depthMapToy = loadTexture("../Resources/toy_box_disp.png");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		rotateCurrent += glm::radians<GLfloat>(deltaTime * 5.0f);
		shader.use();
		glm::mat4 model;
		model = glm::rotate(model, rotateCurrent, glm::normalize(glm::vec3(0.0, 0.0, 1.0)));
		...
		glUniform1i(glGetUniformLocation(shader.program, "texture_depth"), 2);
		...
		glUniform1i(glGetUniformLocation(shader.program, "texture_diffuse"), 0);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, diffuseMapToy);
		glUniform1i(glGetUniformLocation(shader.program, "texture_normal"), 1);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, normalMapToy);
		glUniform1i(glGetUniformLocation(shader.program, "texture_depth"), 2);
		glActiveTexture(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, depthMapToy);
		glBindVertexArray(quadVAO);
		glDrawArrays(GL_TRIANGLES, 0, 6);
		glBindVertexArray(0);
